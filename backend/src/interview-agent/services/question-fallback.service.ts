...
import { FallbackQuestion, FallbackQuestionCategory } from '../entities/fallback-question.entity';
...
export class QuestionFallbackService {
    ...
    async getNextFallbackQuestion(jobRole: string, currentQuestionIndex: number): Promise<string> {
        this.logger.log(`Fetching fallback question for ${jobRole} at index ${currentQuestionIndex}`);
