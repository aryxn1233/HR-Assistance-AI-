...
export class InterviewAgentService {
    ...
    async processAnswer(id: string, answer: string): Promise<any> {
        ...
        if (['completed', 'cancelled', 'failed_interview', 'terminated_by_recruiter'].includes(interview.status)) {
            throw new BadRequestException(`Cannot process answer. Interview is in status: ${interview.status}`);
        }

        // Determine the last question asked (if any)
        const transcript = interview.transcript || [];
        const lastQuestion = transcript.length > 0 ? transcript[transcript.length - 1].question : null;

        // Skip detection logic
        if (this.isSkipResponse(answer)) {
            const { interview: updatedInterview, terminated } = await this.sessionService.incrementSkipCounter(id);
            if (terminated) {
                return { status: updatedInterview.status, message: 'Interview terminated due to multiple skipped questions.' };
            }
        }

        // Add answer to transcript
        if (lastQuestion) {
            interview = await this.sessionService.addTranscriptEntry(id, lastQuestion, answer);
        }
    ...
    private async generateNextQuestion(id: string, interview: any): Promise<string> {
        const transcript = interview.transcript || [];
        const jobRole = interview.job?.title || 'Software Engineer';

        if (this.openAiManager.isFallbackMode || interview.status === 'ai_fallback_mode') {
            return await this.questionFallback.getNextFallbackQuestion(jobRole, interview.currentQuestionIndex);
        }
...
Do not reveal answers. Do not give hints. Do not ask multiple questions.`
        ...
        if (!nextQuestion) {
            this.logger.warn(`Failed to generate question via AI for ${id}. Switching to Fallback Mode.`);
