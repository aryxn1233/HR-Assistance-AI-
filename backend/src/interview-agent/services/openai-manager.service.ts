...
export class OpenAiManagerService {
    ...
    private initializeKeys() {
        ...
        if (this.keys.length === 0) {
            this.logger.warn('No OPENAI API keys found! System will start in fallback mode.');
    ...
    private rotateKey() {
        if (this.currentKeyIndex < this.keys.length - 1) {
            ...
            this.logger.error('All OpenAI API keys have been exhausted! Switching to AI_FALLBACK_MODE.');
            this.isFallbackMode = true;
        }
    }

    async generateNextQuestion(history: { role: 'system' | 'user' | 'assistant', content: string }[], retries = 0): Promise<string | null> {
        ...
        try {
            ...
            if (error?.status === 429 || error?.code === 'insufficient_quota' || error?.message?.includes('rate_limit_error') || error?.message?.includes('quota_exceeded')) {
                this.rotateKey();
    ...
    async evaluateAnswer(question: string, answer: string): Promise<any | null> {
        ...
        try {
            const response = await openAi.chat.completions.create({
                model: 'gpt-3.5-turbo',
                messages: [
                    { role: 'system', content: 'You are an AI technical interviewer evaluating a candidate answer. Return a JSON with { "technicalScore": 1-10, "accuracyScore": 1-10, "communicationScore": 1-10, "confidenceScore": 1-10, "feedback": "Brief feedback" }' },
