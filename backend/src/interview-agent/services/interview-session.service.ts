...
export class InterviewSessionService {
    ...
    async addTranscriptEntry(id: string, question: string, answer: string): Promise<Interview> {
    ...
    async incrementSkipCounter(id: string): Promise<{ interview: Interview, terminated: boolean }> {
        const interview = await this.getInterview(id);
        ...
        if (interview.skipCounter >= MAX_SKIPS) {
            this.logger.warn(`Interview ${id} skipped >= ${MAX_SKIPS} times. Auto-terminating.`);
            ...
            this.liveInterviewGateway.broadcastTermination(id, "FAILED_INTERVIEW", "This interview has ended because multiple questions were skipped.");
            // Notify recruiter dashboard
            this.liveInterviewGateway.broadcastStatus(id, InterviewStatus.FAILED_INTERVIEW);
        }
    ...
    async activateFallbackMode(id: string): Promise<void> {
        ...
        if (interview.status !== InterviewStatus.AI_FALLBACK_MODE) {
            ...
            await this.interviewsRepository.save(interview);
            this.liveInterviewGateway.broadcastStatus(id, InterviewStatus.AI_FALLBACK_MODE);
