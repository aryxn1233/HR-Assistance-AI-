...
export class InterviewGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleJoinRoom(client: Socket, payload: { interviewId: string }) {
        ...
        try {
            ...
            const result = await this.interviewsService.startSession(payload.interviewId);

            if (result.status === 'completed') {
                this.server.to(payload.interviewId).emit('interviewEnd', { message: 'Interview completed', report: result.report });
            } else if (result.question) {
                // Send question text only to client for safety/simplicity
                this.server.to(payload.interviewId).emit('question', { text: result.question.questionText });
            }
        } catch (error) {
    ...
    async handleSubmitAnswer(client: Socket, payload: { interviewId: string; answer: string }) {
        ...
        try {
            ...
            const result = await this.interviewsService.submitAnswer(payload.interviewId, payload.answer);
            ...
            console.error(`Error submitting answer for ${payload.interviewId}:`, error);
