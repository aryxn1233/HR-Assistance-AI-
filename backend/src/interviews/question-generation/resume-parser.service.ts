...
export class ResumeParserService {
    ...
    private extractSkills(text: string): string[] {
        const commonSkills = [
            'javascript', 'typescript', 'react', 'nodejs', 'nextjs', 'nestjs', 'python',
            'java', 'sql', 'postgresql', 'mongodb', 'docker', 'aws', 'azure', 'ai',
            'machine learning', 'blockchain', 'golang', 'rust', 'c++', 'c#', 'php',
            'laravel', 'django', 'flask', 'vue', 'angular', 'redux', 'tailwind',
            'kubernetes', 'terraform', 'graphql', 'rest api', 'microservices'
        ];
    ...
    private extractProjects(text: string): any[] {
        ...
        for (let line of lines) {
            ...
            if (inProjects && line.trim().length > 20 && (line.includes(':') || line.startsWith('-') || line.startsWith('*'))) {
                projects.push({
        ...
        return projects.length > 0 ? projects : [{ name: 'Personal Projects', description: 'Assorted development work' }];
    }
