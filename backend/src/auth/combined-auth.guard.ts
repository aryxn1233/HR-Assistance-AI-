...
export class CombinedAuthGuard extends AuthGuard(['jwt', 'clerk']) {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    console.log(`[CombinedAuthGuard] Checking auth for: ${request.method} ${request.url}`);
    try {
      ...
      console.log(`[CombinedAuthGuard] Auth success for ${request.url}: ${!!finalResult}`);
      return !!finalResult;
    } catch (err) {
      console.error(`[CombinedAuthGuard] Auth FAILED for ${request.url}: ${err.message}`);
      throw err;
    }
  }
}
