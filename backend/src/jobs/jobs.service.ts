...
export class JobsService {
    ...
    async create(createJobDto: any, recruiterId: string): Promise<Job> {
        const job = this.jobsRepository.create({ ...createJobDto, createdBy: recruiterId });
    ...
    async remove(id: string, recruiterId: string, role?: string): Promise<void> {
        console.log(`Attempting to delete job: ${id} by recruiter: ${recruiterId} (Role: ${role})`);
        ...
        if (!isOwner && !isOrphanedRecruiter) {
            console.log(`Permission denied: isOwner=${isOwner}, isOrphanedRecruiter=${isOrphanedRecruiter}`);
