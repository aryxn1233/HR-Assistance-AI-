...
export default function RecruiterMonitorPage() {
    ...
    return (
        <div className="flex flex-col h-screen bg-[#0a0f1c] text-slate-200">
            {/* Top Navigation Bar */}
            <header className="h-auto min-h-20 border-b border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 sm:px-8 bg-[#0a0f1c]/80 backdrop-blur-xl sticky top-0 z-50">
                <div className="flex items-center gap-4 sm:gap-6 w-full sm:w-auto">
                    <Button variant="ghost" size="icon" onClick={() => router.back()} className="rounded-2xl hover:bg-white/5 text-slate-400 shrink-0">
                        <X size={20} />
                    </Button>
                    <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                            <h1 className="text-lg sm:text-xl font-bold tracking-tight truncate">Live Monitor</h1>
                            <Badge className="bg-red-500/10 text-red-500 border-red-500/20 rounded-full flex items-center gap-1.5 px-3 py-0.5 sm:py-1 animate-pulse shrink-0">
                                <div className="w-1.5 h-1.5 bg-red-500 rounded-full" />
                                LIVE
                            </Badge>
                        </div>
                        <p className="text-[10px] text-slate-500 font-medium truncate">ID: {interviewId}</p>
                    </div>
                </div>

                <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                    <div className="hidden xs:flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-2xl border border-white/10">
                    ...
                    <Button
                        ...
                        className="rounded-2xl h-10 sm:h-12 px-4 sm:px-6 bg-red-600 hover:bg-red-700 font-bold shadow-xl shadow-red-900/20 flex gap-2 text-xs sm:text-sm"
                    >
                        {isTerminating ? <Loader2 className="animate-spin h-4 w-4" /> : <Power size={18} className="hidden sm:block" />}
                        Terminate Session
                    </Button>
                </div>
            </header>
            ...
            <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                ...
                <aside className="w-full lg:w-[450px] h-[400px] lg:h-full border-t lg:border-t-0 lg:border-l border-white/5 bg-[#0a0f1c] flex flex-col shrink-0">
                    <div className="p-4 sm:p-8 border-b border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="p-2 sm:p-2.5 bg-blue-500/10 rounded-xl">
                                <MessageSquare size={16} className="text-blue-500" />
                            </div>
                            <h2 className="font-bold text-base sm:text-lg text-white">Live Transcript</h2>
                        </div>
                        <Badge variant="outline" className="rounded-full border-white/10 text-[10px] text-slate-400">Real-time sync</Badge>
                    </div>
